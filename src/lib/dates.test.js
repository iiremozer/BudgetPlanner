...
describe('formatDayLabel', () => {
  ...
  it('bugünü ve dünü adlandırır', () => {
    expect(formatDayLabel('2026-03-10', now)).toBe('Bugün');
    expect(formatDayLabel('2026-03-09', now)).toBe('Dün');
  });

  it('aynı yıl içinde gün adını ekler', () => {
    expect(formatDayLabel('2026-03-07', now)).toBe('7 Mart, Cumartesi');
  });

  it('geçmiş yıllarda yılı yazar', () => {
    expect(formatDayLabel('2025-12-24', now)).toBe('24 Aralık 2025');
  });

  it('ay sınırında dünü doğru bulur', () => {
    expect(formatDayLabel('2026-02-28', day(2026, 3, 1))).toBe('Dün');
  });

  it('bozuk anahtarda boş döner', () => {
    expect(formatDayLabel('yarın', now)).toBe('');
  });
});
